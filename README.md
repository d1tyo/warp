...
Multipart upload benchmark tests upload speed of parts. It creates multipart upload, uploads parts of `--part.size` 
size each so the total object size is `--obj.size` (therefore `--obj.size` must be a multiple of `--part.size`)
and completes multipart upload when all parts are uploaded.
...
```
TODO(dtyo): use log from VM, because now the numbers are just a shame... 
